    ...
    #' 
{
    ...
    for(i in 1:k)
    {
        Respons[,i] <- p[i]*dmnorm(x, mean = Mu[i,], varcov = Sigma[,,i], log=FALSE)
          ## Complete the code
    }
    ...
    #' returns the optimized parametres (Mu, Sigma, p) as a list 
{
    ...
    d <- ncol(x)
    k <- ncol(respons)
    NK <- apply(respons, 2, sum) ## sum by column
    p <-  NK/N ## Complete the code 
    mustar <- t(respons) %*% x / NK ## Complete the code 
    Sigstar=array(dim=c(d,d,k))
    for( i in (1:k)){
        ## Complete the code
        temps <- t(t(x) - mustar[i,]) # sub by column
        Sigstar[,,i] <- 1/NK[i] * t(temps) %*% (temps * respons[,i])
          ## Complete the code
    }
...
{ 
    ...
    {
        niter <- niter+1
        respons <- estep(x = x,Mu = current$Mu, Sigma = current$Sigma, p = current$p ) ## responsibilities (n*k matrix)
            ## complete the code
            current <- mstep(x = x, respons = respons ) ## New parameters (complete using M-step)
    ...
    #' Beta: k-vector                   
{
    ...
    for( j in (1:K)){
        ...
        Winv[,,j] <- W0inv + NK[j] * Sj + beta0 * NK[j] / (beta0 + NK[j]) * (barx[j,] - m0) %*% t((barx[j,] - m0)) ## complete the code: optimal W^{-1}
...
{
    d <-  ncol(M) # get the d: dimension
    k <- length(alpha) # get the k : number of class, degrees of freedom
    ...
    #'   where T is the number of steps.
{
    ...
    while(continue){        
        niter <- niter+1
        respons <- 
          vbEstep(x = x, alpha = current$alpha, Winv = current$Winv, Nu = current$Nu, M = current$M, Beta = current$Beta ) 
        ## Complete the code using vbEstep
        
        if(nanDetector(respons)) {stop("NaNs detected!\n")}
        
        vbOpt <- vbMstep(x = x, respons = respons, alpha0 = alpha0, W0inv = W0inv, nu0 = nu0, m0 = m0, beta0 = beta0)
          ## Complete the coe using vbMstep
...
rproposal <- function( Mu, Sigma, p, ppar=list(var_Mu = 0.1,
    ...
    #' - alpha_p: concentration parameter for the Dirichlet kernel for p
    #' returns: a list of proposal parameters: (Mu, Sigma, p), where 
    ...
    #' - naccept! number of accepted proposals. 
{
    ...
    for (niter in 1:nsample){
        ...
        llbackp <- ddirichlet(current$p ,alphaPropback,log = TRUE)
        llmovep <- ddirichlet(proposal$p ,alphaPropmove, log = TRUE)
        
        llbackMu <- sum(vapply(1:k, FUN = function(j){
          dmvn(x = current$Mu[j,], mu = proposal$Mu[j,], 
               Sigma = proposal$Sigma[,,j], log=TRUE)},
          FUN.VALUE = numeric(1)))
        
        llmoveMu <- sum(vapply(1:k, FUN = function(j){
          dmvn(x = proposal$Mu[j,], mu = current$Mu[j,], 
               Sigma = current$Sigma[,,j], log=TRUE)},
          FUN.VALUE = numeric(1)))
        
        lacceptratio <- min(0, proposal$lpost - current$lpost + llbackp - llmovep + llbackMu - llmoveMu + llkbackSigma - llkmoveSigma  )
    ...
    #' returns: a single numeric value
{
    ...
    vectllk <- vapply(inds, function(niter){
        ## complete the code
        #T <- ncol(sample$p)
        exp(gmllk(x, Mu = sample$Mu[,,niter],
                  Sigma = sample$Sigma[,,,niter], p = sample$p[,niter]))
      },
        FUN.VALUE = numeric(1))
    return(mean(vectllk))
}
